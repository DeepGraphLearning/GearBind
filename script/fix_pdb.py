...
def fix_pdb(pdbfile, alterations_info, output_file=None, keep_water=False, pH=7.0):
    ...
    Returns:
        If output_file is None, a PDB string representing the fixed structure.
        Otherwise, the output_file is written to and the function returns None.
    """
    if isinstance(pdbfile, (str, bytes, os.PathLike)):
        with open(pdbfile, 'r') as in_handle:
            fixer = pdbfixer.PDBFixer(pdbfile=in_handle)
    elif isinstance(pdbfile, io.IOBase):
        fixer = pdbfixer.PDBFixer(pdbfile=pdbfile)
    else:
        raise ValueError('pdb_file must be a file handle or path')
    ...
    tmp_handle = io.StringIO()
    app.PDBFile.writeFile(fixer.topology, fixer.positions, tmp_handle, keepIds=True)
    tmp_handle.seek(0)

    pdb_structure = PdbStructure(tmp_handle)
    _clean_structure(pdb_structure, alterations_info)

    if output_file is None:
        out_handle = io.StringIO()
        app.PDBFile.writeFile(fixer.topology, fixer.positions, out_handle, keepIds=True)
        return out_handle.getvalue()
    elif isinstance(output_file, (str, bytes, os.PathLike)):
        with open(output_file, 'w') as out_handle:
            app.PDBFile.writeFile(fixer.topology, fixer.positions, out_handle, keepIds=True)
    elif isinstance(output_file, io.IOBase):
        app.PDBFile.writeFile(fixer.topology, fixer.positions, output_file, keepIds=True)
    else:
        raise ValueError('output_file must be a file handle, a path or None')


def _clean_structure(pdb_structure, alterations_info):
